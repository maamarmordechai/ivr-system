-- Complete setup for bed confirmation tracking system
-- Run ALL of these in order in Supabase SQL Editor

-- 1. Create bed_confirmations table
...
-- 2. Enable wants_weekly_calls for existing apartments
-- Update all apartments to receive weekly calls by default
UPDATE apartments 
SET wants_weekly_calls = true 
WHERE wants_weekly_calls IS NULL OR wants_weekly_calls = false;

-- 3. Verify setup
SELECT 
  'Apartments wanting weekly calls' as info,
  COUNT(*) as count
FROM apartments 
WHERE wants_weekly_calls = true;

SELECT 
  'Bed confirmations table exists' as info,
  COUNT(*) as rows
